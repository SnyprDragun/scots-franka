...
auto system_post = [](state_type &x, const input_type &u) noexcept {
   ...
   state_type z=x;
   x[0]=pow(2,input_dim)*z[0]+pow(2,input_dim-1)*u[0]+pow(2,input_dim-2)*u[1]+pow(2,input_dim-3)*u[2]+pow(2,input_dim-4)*u[3]+pow(2,input_dim-5)*u[4]+pow(2,input_dim-6)*u[5]+pow(2,input_dim-7)*u[6]+pow(2,input_dim-8)*u[7];
   x[1]=pow(0.5,input_dim)*z[1]+pow(0.5,input_dim-1)*u[0]+pow(0.5,input_dim-2)*u[1]+pow(0.5,input_dim-3)*u[2]+pow(0.5,input_dim-4)*u[3]+pow(0.5,input_dim-5)*u[4]+pow(0.5,input_dim-6)*u[5]+pow(0.5,input_dim-7)*u[6]+pow(0.5,input_dim-8)*u[7];
};
...
auto radius_post = [](state_type &r, const state_type&, const input_type &u) noexcept {
	...
	state_type zz=r;
	r[0]=pow(2,input_dim)*zz[0];
    r[1]=pow(0.5,input_dim)*zz[1];
};
...
int main() {
 ...
 /* lower bounds of the hyper-rectangle */
  ...
  /* lower bounds of the hyper rectangle */
  input_type i_lb={{-1,-1,-1,-1,-1,-1,-1,-1}};
  //input_type i_lb={{-1}};
  /* upper bounds of the hyper rectangle */
  input_type i_ub={{1,1,1,1,1,1,1,1}};
  //input_type i_ub={{1}};
  /* grid node distance diameter */
  input_type i_eta={{1,1,1,1,1,1,1,1}};
